...
case "$os" in
  ...
  *) die "unsupported OS: $os (diagprint's install script supports Linux and macOS; try 'cargo install diagprint' instead)" ;;
esac
...
archive="diagprint-${target}.tar.gz"
...
install -m 755 "${tmp_dir}/diagprint" "${INSTALL_DIR}/diagprint"

echo ""
echo "diagprint installed to ${INSTALL_DIR}/diagprint"
case ":$PATH:" in
  *":$INSTALL_DIR:"*) ;;
  *) echo "Note: ${INSTALL_DIR} is not on your PATH. Add it, e.g.:" ;
     echo "  export PATH=\"${INSTALL_DIR}:\$PATH\"" ;;
esac
echo "Run 'diagprint --help' to get started."
